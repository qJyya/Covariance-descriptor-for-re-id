...
def eigenvalues(K):
    ...
    # B=L^-1*K
    B = torch.matmul(L_inv.to(device),K.to(device))
    
    B.to(device)
    
    A = torch.bmm(L_inv.to(device),(torch.transpose(B,1,2)).to(device))
    A.to(device)
    ...
    # since = time.time()
    λ = (torch.linalg.eigvals(A).real).to(device)

    # time_elapsed = time.time() - since
    # print('time: {:.0f}m {:.2f}s'.format(
    #         time_elapsed // 60, time_elapsed % 60))
    return λ
    
#######################################################################
# Evaluate
def evaluate(qf,ql,qc,gl,gc):
    # query = qf.to(device)
    # n=gf.shape[0]
    # dist_list=[]
    # for i in range(n):
    ...
    λ=(eigenvalues(qf.to(device))).to(device)
...
result = scipy.io.loadmat('city1m_part1_resize_cut.mat')
...
L=torch.linalg.cholesky(gallery_feature.to(device), upper=False)
...
for i in range(len(query_label)):
    ap_tmp, CMC_tmp = evaluate(query_feature[i].to(device),query_label[i],query_cam[i],gallery_label,gallery_cam)
    if CMC_tmp[0]==-1:
        continue
    CMC = CMC + CMC_tmp
    ap += ap_tmp
