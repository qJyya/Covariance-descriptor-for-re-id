...
def extract_feature(dataloaders):
    ...
    for iter,data in enumerate(dataloaders):
        ...
        count=0
        n, c, h, w = imgs.size() 
        img_feature=torch.DoubleTensor(h*w,13).zero_().to(device)
        ...
        for k in range(n):
            ...
            for i in range(h):
                for j in range(w):
                    ...
                    E_matrix=torch.mm(matrix,rgb_matrix).to(device)
            ...
            for i in range(1,h-1):
                for j in range(1,w-1):
                    pixel_feature=torch.DoubleTensor(1,13).to(device)
                    R_feature=img[0,:,:]
                    G_feature=img[1,:,:]
                    B_feature=img[2,:,:]
                    ...
                    pixel_feature.index_fill_(1, torch.tensor(0).to(device), i+r)
                    pixel_feature.index_fill_(1, torch.tensor(1).to(device), j+r)
                    pixel_feature.index_fill_(1, torch.tensor(2).to(device), div_1+r)
                    pixel_feature.index_fill_(1, torch.tensor(3).to(device), div_2+r)
                    pixel_feature.index_fill_(1, torch.tensor(4).to(device), E_x1)
                    pixel_feature.index_fill_(1, torch.tensor(5).to(device), E_y1+r)
                    pixel_feature.index_fill_(1, torch.tensor(6).to(device), arctan_1+r)
                    pixel_feature.index_fill_(1, torch.tensor(7).to(device), E_x2)
                    pixel_feature.index_fill_(1, torch.tensor(8).to(device), E_y2+r)
                    pixel_feature.index_fill_(1, torch.tensor(9).to(device), arctan_2+r)
                    pixel_feature.index_fill_(1, torch.tensor(10).to(device), E_x3)
                    pixel_feature.index_fill_(1, torch.tensor(11).to(device), E_y3+r)
                    pixel_feature.index_fill_(1, torch.tensor(12).to(device),arctan_3+r)
                    img_feature[i:i+1,:]=pixel_feature

            #计算协方差矩阵 
            cov_feature=torch.cov(img_feature.T).to(device)
    ...
    imgs_feature=torch.stack(imgs_feature_list,dim=0).to(device)
